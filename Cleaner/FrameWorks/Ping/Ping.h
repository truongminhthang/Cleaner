...
/*! Controls the IP address version used by Ping instances.
 */

typedef NS_ENUM(NSInteger, PingAddressStyle) {
    PingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    AddressStyleICMPv4,       ///< Use the first IPv4 address found.
    PingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@property (nonatomic, weak, readwrite, nullable) id<PingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) PingAddressStyle addressStyle;
...
/*! A delegate protocol for the Ping class.
 */
...
/*! A Ping delegate callback, called once the object has started up.
...
/*! A Ping delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
/*! A Ping delegate callback, called when the object has successfully sent a ping packet.
 ...
 *      considered API.
 *  \param pinger The object issuing the callback.
 *  \param packet The packet that was sent; this includes the ICMP header (`ICMPHeaderPing`) and the 
...
- (void)ping:(Ping *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;

/*! A ping delegate callback, called when the object fails to send a ping packet.
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-ping:didSendPacket:sequenceNumber:` delegate callback or a
 *      `-ping:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
- (void)ping:(Ping *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;

/*! A Ping delegate callback, called when the object receives a ping response.
 ...
 *  \param pinger The object issuing the callback.
 *  \param packet The packet received; this includes the ICMP header (`ICMPHeaderPingPing`) and any data that 
 *      follows that in the ICMP message but does not include any IP-level headers.
 *  \param sequenceNumber The ICMP sequence number of that packet.
 */

- (void)ping:(Ping *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;

/*! A Ping delegate callback, called when the object receives an unmatched ICMP message.
 ...
 *  \param pinger The object issuing the callback.
 *  \param packet The packet received; this includes the ICMP header (`ICMPHeaderPing`) and any data that 
 *      follows that in the ICMP message but does not include any IP-level headers.
 */
 ...
 *  
 *      This is declared in the header because clients of Ping might want to use
 *      it parse received ping packets.
 */

struct ICMPHeaderPing {
    uint8_t     type;
    uint8_t     code;
    uint16_t    checksum;
    uint16_t    identifier;
...
typedef struct ICMPHeaderPing ICMPHeaderPing;

__Check_Compile_Time(sizeof(ICMPHeaderPing) == 8);
__Check_Compile_Time(offsetof(ICMPHeaderPing, type) == 0);
__Check_Compile_Time(offsetof(ICMPHeaderPing, code) == 1);
__Check_Compile_Time(offsetof(ICMPHeaderPing, checksum) == 2);
__Check_Compile_Time(offsetof(ICMPHeaderPing, identifier) == 4);
__Check_Compile_Time(offsetof(ICMPHeaderPing, sequenceNumber) == 6);
